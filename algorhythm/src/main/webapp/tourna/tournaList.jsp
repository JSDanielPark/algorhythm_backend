										...
										$(document).ready(function() {
											...
											$.ajax(req).done(function(data, status) {
												...
												$( "tbody" ).append('<tr><td class="text-center">'
														+ data.list[i].Tourna_No + '</td><td class="text-center"><a href="/tourna/tournaDetail.jsp?page='
														+ page +'&tourna_no='
														+ data.list[i].Tourna_No +'">'
														+ data.list[i].Title + '</a></td><td class="text-Left">'
														+ data.list[i].Reg_Date + '</td></tr>');
												}
											}).error(function() {
												alert("오류가 발생했습니다.");
											});
